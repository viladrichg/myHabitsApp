...
import { DailyEntry, AppSettings, CustomSport, UserProfile, DisplayMode } from '../database/types';
...
/**
 * Returns the current displayMode from persisted settings ('absolute' | 'percentage').
 * Defaults to 'absolute' while settings are loading.
 *
 * WHY this lives in the data layer:
 *   - UI components are pure renderers; they must not contain conversion logic.
 *   - Centralising the conversion here means tables, stats, and charts all
 *     receive the same normalised values without per-component coupling.
 */
export const useDisplayMode = (): DisplayMode => {
  const { data: settings } = useSettings();
  return settings?.displayMode ?? 'absolute';
};

/**
 * Converts a raw cumulative count into the display value selected by the user.
 *
 * @param rawCount  - number of days the field was active
 * @param totalDays - number of days with a recorded entry in the range
 * @returns         - rounded display value (count or percentage 0-100)
 *
 * This is the single normalisation point. No screen or component should
 * perform this arithmetic independently.
 */
export const normalizeDisplayValue = (
  ...
  if (mode === 'percentage') {
    ...
    return Math.round((rawCount / totalDays) * 100);
  }
  return rawCount;
};

/**
 * Hook that returns a bound normaliser using the current displayMode.
 * Usage in a component:
 *   const normalize = useNormalizeValue(totalTrackedDays);
 *   const display = normalize(rawCount); // "12" or "40%"
 */
export const useNormalizeValue = (totalDays: number) => {
  const mode = useDisplayMode();
  return useMemo(
    () => (rawCount: number) => normalizeDisplayValue(rawCount, totalDays, mode),
    [mode, totalDays]
  );
};

/**
 * Returns the unit suffix appropriate for the current display mode.
 * Components can append this to displayed values without knowing the mode.
 */
export const useDisplayUnit = (): string => {
  const mode = useDisplayMode();
