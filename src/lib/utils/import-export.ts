...
export async function importFromCSV(
  ...
  try {
    ...
    // Strict header validation when labels are provided
    if (labels) {
      const missingCols = validateVariableHeaders(headers, labels);
      if (missingCols.length > 0) {
        result.errors.push('CSV headers do not match current variable names.');
        result.errors.push(
          `Expected headers: Data,Wakeup,SleepQuality,Bedtime,${labels.workedAtJob},${labels.workedAtHome},${labels.fum},${labels.gat},${labels.meditation},${labels.yoga},${labels.dibuix},${labels.llegir},${labels.sports},Notes`
        );
        result.errors.push(`Missing columns: ${missingCols.join(', ')}`);
        result.errors.push('Please rename the CSV columns to match the variable names in Settings → Variables & Colors.');
        return result;
      }
    }

    const columnMap = buildColumnMap(labels);
...
export function parseFileForConflicts(
  ...
  if (fileType === 'json') {
    ...
    try {
      ...
      // If labels provided, validate headers before parsing
      if (labels) {
        const missingCols = validateVariableHeaders(headers, labels);
        if (missingCols.length > 0) {
          errors.push('CSV headers do not match current variable names.');
          errors.push(
            `Expected: Data,Wakeup,SleepQuality,Bedtime,${labels.workedAtJob},${labels.workedAtHome},${labels.fum},${labels.gat},${labels.meditation},${labels.yoga},${labels.dibuix},${labels.llegir},${labels.sports},Notes`
          );
